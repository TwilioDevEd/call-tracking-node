...
describe('Lead controllers', function() {
  ...
  describe('POST /lead', function() {
    ...
    it('forwards the call to a different number', function(done) {
      ...
      newLeadSource.save().then(function() {
        agent
          .post('/lead')
          .type('form')
          .send({
          ...
          .expect(function(response) {
            var twiml = '<Dial>' 
              + leadSourceForwardingNumber.toString() 
              + '</Dial>';
    ...
    it('records a new lead', function(done) {
      ...
      newLeadSource.save().then(function(savedLeadSource) {
        agent
          .post('/lead')
          .type('form')
          .send({
          ...
          .expect(function(response) {
            Lead.findOne({}).then(function(newLead) {
              expect(newLead.callerNumber).to.equal(callerNumber);
              expect(newLead.leadSource.toString())
                .to.equal(savedLeadSource._id.toString());
  ...
  describe('statistics for pie charts', function() {
    ...
    it('responds with stats for a leads by lead source chart', function(done) {
      var newLeadSourceOne = new LeadSource({
        number: leadSourceOneNumber,
        description: leadSourceOneDescription,
        forwardingNumber: leadSourceOneForwardingNumber
      }).save();

      var newLeadSourceTwo = new LeadSource({
        number: leadSourceTwoNumber,
      ...
      q.all([
        ...
        var agent = supertest(app);
        agent
          .get('/lead/summary-by-lead-source')
          .expect(function(response) {
    ...
    it('responds with statistics for a leads by city chart', function(done) {
      ...
      newLeadSource.then(function(savedLeadSource) {
        newLead.leadSource = savedLeadSource._id;
        return newLead.save();
      }).then(function(savedLead) {
        var agent = supertest(app);
        agent
          .get('/lead/summary-by-city')
          .expect(function(response) {
